...
public class UserServiceImpl implements UserService{
	...
	public User login(String name, String password) {
		...
		try {
			return queryRunner.query(SQL,new BeanHandler<User>(User.class),new Object[] {name,password});
	...
	public Page<User> query(UserQuery userQuery) {
		...
		try {
			page.setDatas(queryRunner.query(SQL, new BeanListHandler<User>(User.class),params.toArray()));
		} catch (SQLException e) {
			e.printStackTrace();
		}
		page.setPageSize(10);
		...
		//当前查询条件下的记录数
		int count = findCount("select count(*) from user "+userQuery.getWhereSQL(),userQuery.getWhereParams());
		page.setCount(count);
		return page;
	}
	...
	public int add(User user) {
		...
		//password 默认值
//		user.setPassword(SysUtil.getValue("password"));
		
		user.dualDynamicSQL();
		
		Connection conn = JDBCUtil.getConn();
		 
		String SQL = "insert into user("+user.getColumnPlaceholder()+") values ("+user.getParamPlaceholder()+")";
	...
	public int update(User user) {
		 QueryRunner queryRunner = new QueryRunner(JDBCUtil.getDataSource());
		    ...
		    try {
				return queryRunner.update(SQL,new Object[] {user.getPassword(),user.getAddress(),user.getPhone(),user.getId()});
			} catch (SQLException e) {
				e.printStackTrace();
			}
			return -1;
