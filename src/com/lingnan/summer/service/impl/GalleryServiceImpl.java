...
public class GalleryServiceImpl implements KitchenService{
	...
	@Override
	public Page<Device> query(KitchenQuery kitchenQuery) {
		
		kitchenQuery.init();
		
		Page<Device> page= new Page<Device>();
		
		String SQL = "select * from device where location='阳台'"+kitchenQuery.getWhereSQL();
		
		List<Object> params = kitchenQuery.getWhereParams();//获取查询参数
		...
		//当前查询条件下的记录数
		int count = findCount("select count(*) from device "+kitchenQuery.getWhereSQL(),kitchenQuery.getWhereParams());
