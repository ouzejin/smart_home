...
public class BedServiceImpl implements BedService{
	...
	@Override
	public Page<Device> query(BedQuery bedQuery) {
		
		bedQuery.init();
		
		Page<Device> page= new Page<Device>();
		
		String SQL = "select * from device where location='卧室'"+bedQuery.getWhereSQL();
		
		List<Object> params = bedQuery.getWhereParams();//获取查询参数
		...
		//当前查询条件下的记录数
		int count = findCount("select count(*) from device where location='卧室' "+bedQuery.getWhereSQL(),bedQuery.getWhereParams());
