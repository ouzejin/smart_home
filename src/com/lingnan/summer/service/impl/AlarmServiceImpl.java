...
public class AlarmServiceImpl implements AlarmService {
	...
	public Page<Alarm> query(AlarmQuery alarmQuery) {
		...
		try {
			page.setDatas(queryRunner.query(SQL, new BeanListHandler<Alarm>(Alarm.class), params.toArray()));
		...
		//当前查询条件下的记录数
		int count = findCount("select count(*) from alarm "+alarmQuery.getWhereSQL(),alarmQuery.getWhereParams());
	...
	public int add(Alarm alarm) {
	    ...
	    String SQL = "insert into alarm("+alarm.getColumnPlaceholder()+") values ("+alarm.getParamPlaceholder()+")";
	    try {
	...
	public int deleteById(int id) {
		QueryRunner queryRunner = new QueryRunner(JDBCUtil.getDataSource());
		String SQL = "delete from alarm where did = ?";
		try {
	...
	public Alarm findAlarmById(int id) {
		QueryRunner queryRunner = new QueryRunner(JDBCUtil.getDataSource());
		String SQL = "select * from alarm where did = ?";
		try {
			return queryRunner.query(SQL,new BeanHandler<Alarm>(Alarm.class),id);
		} catch (SQLException e1) {
			e1.printStackTrace();
		}
		return null;
	...
	public int update(Alarm alarm) {
		QueryRunner queryRunner = new QueryRunner(JDBCUtil.getDataSource());
	    String SQL = "update alarm set dname=?,risk=?,measure=? where did = ?";
	    try {
			return queryRunner.update(SQL,new Object[] {alarm.getDname(),alarm.getRisk(),alarm.getMeasure(),alarm.getDid()});
		} catch (SQLException e) {
			e.printStackTrace();
		}
		return -1;
