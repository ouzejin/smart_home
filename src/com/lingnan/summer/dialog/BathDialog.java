...
public class BathDialog extends JDialog implements ActionListener {
	...
	public BathDialog(JFrame parent, BathView bathView) {
		super(parent, "添加");
	...
	public BathDialog(JFrame parent, BathView bathView, Device device) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (source == saveBtn) {
			...
			if (this.device == null) {
				...
				int result = bathService.add(device);
				if (result == 1) {
					this.dispose();
				...
				int result = bathService.update(device);
				if (result == 1) {
