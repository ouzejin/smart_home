...
public class BedDialog extends JDialog implements ActionListener {
	...
	public BedDialog(JFrame parent, BedView bedView) {
		super(parent, "添加");
	...
	public BedDialog(JFrame parent, BedView bedView, Device device) {
		this(parent, bedView);
		this.device = device;
		setTitle("修改");
		// * //回显
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (source == saveBtn) {
			...
			if (this.device == null) {
				...
				int result = bedService.add(device);
				if (result == 1) {
					this.dispose();
				...
				int result = bedService.update(device);
				if (result == 1) {
